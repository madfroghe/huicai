...
(function($, window, document, undefined) {
	...
	Picker.prototype.setItems = function(items) {
		var self = this;
		self.items = items.regionList||items.storeList|| [];
		var buffer = [];
		self.items.forEach(function(item) {
			item.type = items.regionType;
			if (item !== null && item !== undefined) {
				buffer.push('<li style="font-size:0.8rem">' + (item.name ||item.store_name|| item) + '</li>');
			}
		});
...
function getRegionData(data){
	var result = {}
	var caode = data.item.caode ||  data.item.code; 
	var cityConvert ={"province":"city","city":"county","county":"town","town":"village"}
	var url = getUrlParam()+"/rest/o2o/api/wy_adds/regionList";
	var querydata = {
	    	regionCode:caode,
	    	regionType:cityConvert[data.item.type]
	   };
	   
	if(data.item.type =="village"){
		url = getUrlParam()+"/rest/o2o/api/wy_adds/wyStoreList"
		querydata = {wyAddressId:data.item.id}
	}
			...
			for (var i = 1; i <= layer; i++) {
				...
				pickerElement.addEventListener('change', function(event) {
					...
					if (nextPickerElement && nextPickerElement.picker) {
						...
						nextPickerElement.picker.setItems(getRegionData(eventData));
					}
				}, false);
			}
		},
